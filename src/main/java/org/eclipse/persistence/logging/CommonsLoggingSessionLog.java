 ...
 * used when messages need to be logged through apache commons logging 1.1.
 * <p/>
 * History :
 * <p/>
...
public final class CommonsLoggingSessionLog extends AbstractSessionLog {
     ...
     */
    public static final boolean FORCE_INTERNAL_DEBUG_STACK = true;
    /**
     ...
     */
    public static final String SESSION_ECLIPSELINK_NAMESPACE = ECLIPSELINK_NAMESPACE
            + ".session";
    /**
     ...
     * logging levels.
     */
    public static final Level[] JAVA_LEVELS = new Level[]{Level.ALL,
     ...
     * thread-safe !
     */
    private final Map<String, String> NAMESPACE_MAP = new HashMap<String, String>(
            32);
    /**
     ...
     */
    private final Map<String, LogWrapper> CATEGORY_LOGGERS = new HashMap<String, LogWrapper>(
            32);
    /**
     ...
     * <p/>
     * Used by : org.eclipse.persistence.internal.jpa.EntityManagerSetupImpl.
     * updateLoggers : creates an instance for singletonLog and sessionLog
     * <p/>
    ...
    public static final void onInit() {
        ...
        if (FORCE_APACHE_COMMONS_LOGGING_DIAGNOSTICS) {
             ...
             * assumed to be the name of a file. The special strings STDOUT or
             * STDERR (case-sensitive) indicate output to System.out and
             ...
             */
            System.setProperty("org.apache.commons.logging.diagnostics.dest",
                    "STDOUT");
        }
    }
    ...
    /**
     * PUBLIC: Return the effective log level for the name space extracted from
     * session and category. If a Logger's level is set to be null then the
     * Logger will use an effective Level that will be obtained by walking up
    ...
    public final int getLevel(final String category) {
        if (FORCE_INTERNAL_DEBUG) {
            debug("CommonsLoggingSessionLog.getLevel : IN : category : "
                    + category);
        }
        ...
        if (FORCE_INTERNAL_DEBUG) {
            debug("CommonsLoggingSessionLog.getLevel : OUT : category : "
                    + category + " : level : " + getLevelString(l));
        }
    ...
    /**
     * PUBLIC: Set the log level to a logger with name space extracted from the
     * given category.
     *
    ...
    public final void setLevel(final int level, final String category) {
        AccessController.doPrivileged(new PrivilegedAction<Object>() {
            ...
            public Object run() {
                if (FORCE_INTERNAL_DEBUG) {
                    debug("CommonsLoggingSessionLog.setLevel : IN : category : "
                            + category + " to level : " + getLevelString(level),
                            true);
                }
                ...
                if (lw == null) {
                    error("CommonsLoggingSessionLog.setLevel : category not found : "
                            + category);
                } else {
                    final Logger logger = getLog4JLogger(lw.getLog());

                    if (logger == null) {
                        error("CommonsLoggingSessionLog.setLevel : Logger not found : "
                                + category + " : " + lw.getLog());
                    } else {
                        ...
                        if (realLevel == org.apache.log4j.Level.OFF) {
                            // force level to OFF :
                            lw.setLevel(OFF);
                            error("CommonsLoggingSessionLog.setLevel : unknown level : "
                        ...
                        if (logger.isEnabledFor(org.apache.log4j.Level.WARN)) {
                            logger.debug("CommonsLoggingSessionLog.setLevel : Logger Level set to : "
                                    + logger.getLevel());
                        }

                        if (FORCE_INTERNAL_DEBUG) {
                            debug("CommonsLoggingSessionLog.setLevel : OUT : category : "
                                    + category
                                    + " to level : "
                                    + logger.getLevel());
                        }
    ...
    /**
     * PUBLIC: Set the output stream that will receive the formatted log
     * entries. DO nothing as Log4J manages the appenders (console or files) via
     * Log4J configuration
     *
    ...
    public final void setWriter(final OutputStream fileOutputStream) {
        if (FORCE_INTERNAL_DEBUG) {
            debug("CommonsLoggingSessionLog.setWriter : stream : "
                    + fileOutputStream);
        }
    ...
    public final void setSession(final Session pSession) {
        ...
        if (pSession != null) {
            ...
            if ((sessionName != null) && (sessionName.length() != 0)) {
                this.sessionNameSpace = SESSION_ECLIPSELINK_NAMESPACE + "."
                        + sessionName;
            } else {
            ...
            if (FORCE_INTERNAL_DEBUG) {
                debug("CommonsLoggingSessionLog.setSession : sessionNameSpace : "
    ...
    /**
     * PUBLIC: Check if a message of the given lev would actually be logged by
     * the logger with name space built from the given session and category.
     * Return the shouldLog for the given category Note : this method is very
     * very used so optimized for performance.
     *
     * @param level    value according to the java.util.logging.Levels
     * @param category category
    ...
    public final boolean shouldLog(final int level, final String category) {
        if (FORCE_INTERNAL_DEBUG) {
            debug("CommonsLoggingSessionLog.shouldLog : IN : category : "
                    + category + " : " + getLevelString(level));
        }
        ...
        switch (level) {
            ...
            default:
                ...
                if (lw == null) {
                    error("CommonsLoggingSessionLog.shouldLog : category : "
                            + category + " - NO LOGGER FOUND");
                    res = false;
                } else {
        ...
        if (FORCE_INTERNAL_DEBUG) {
            debug("CommonsLoggingSessionLog.shouldLog : OUT : category : "
                    + category + " : " + res);
        }
    ...
    public final void log(final SessionLogEntry entry) {
        if (shouldLog(entry.getLevel(), entry.getNameSpace())) {
            if (FORCE_INTERNAL_DEBUG) {
                debug("CommonsLoggingSessionLog.log : namespace : "
                        + entry.getNameSpace() + " message : "
                        + entry.getMessage());
            }
            ...
            if (log == null) {
                error("CommonsLoggingSessionLog.log : no Log found for : "
                        + entry.getNameSpace());
            } else {
    ...
    /**
     * INTERNAL: Each session owns its own session log because session is stored
     * in the session log
     *
     * @return value
     */
    ...
    public final Object clone() {
        ...
        // The state of this object is described by member variables
        // sessionLogger and categoryLoggers.
        // This state depends on session.
        // If session for the clone is going to be the same as session for this
        // there is no
        // need to do "deep" cloning.
        // If not, the session being cloned should call setSession() on its
        // JavaLog object to initialize it correctly.
        final CommonsLoggingSessionLog cloneLog = (CommonsLoggingSessionLog) super
                .clone();

        if (FORCE_INTERNAL_DEBUG) {
            debug("CommonsLoggingSessionLog.clone : cloned instance : "
                    + cloneLog, true);
        }
     ...
     */
    private final void addLogger(final String loggerCategory,
                                 final String loggerNameSpace) {
        if (FORCE_INTERNAL_DEBUG) {
            debug("CommonsLoggingSessionLog.addLogger : category : "
                    + loggerCategory + " in name space : " + loggerNameSpace);
        }

        this.CATEGORY_LOGGERS.put(loggerCategory, new LogWrapper(this,
                loggerCategory, LogFactory.getLog(loggerNameSpace)));
    }
    ...
    /**
     * INTERNAL: Return the LogWrapper instance for the given category Note :
     * this method is very very used so optimized for performance.
     *
     * @param category category
    ...
    private final LogWrapper getLogWrapper(final String category) {
        ...
        if (getSession() == null) {
            if (FORCE_INTERNAL_DEBUG) {
                debug("CommonsLoggingSessionLog.getLogWrapper : " + category
                        + " : SESSION NULL");
            }
            ...
            if (lw == null) {
                if (FORCE_INTERNAL_DEBUG) {
                    debug("CommonsLoggingSessionLog.getLogWrapper : "
    ...
    /**
     * INTERNAL: Return the apache commons logging Log instance for the given
     * category
     *
     * @param category category
    ...
    private final Log getLog(final String category) {
        if (FORCE_INTERNAL_DEBUG) {
            debug("CommonsLoggingSessionLog.getLogger : IN : category : "
                    + category);
        }
    ...
    private final void addDefaultLoggers(final String namespace) {
        if (FORCE_INTERNAL_DEBUG) {
            debug("CommonsLoggingSessionLog.addDefaultLoggers : nameSpace : "
                    + namespace);
        }
        ...
        if (FORCE_INTERNAL_DEBUG) {
            debug("CommonsLoggingSessionLog.addDefaultLoggers : NAMESPACE_MAP : "
                    + this.NAMESPACE_MAP);
            debug("CommonsLoggingSessionLog.addDefaultLoggers : CATEGORY_LOGGERS : "
                    + this.CATEGORY_LOGGERS);
        }
    }
     ...
     */
    private final void internalLog(final SessionLogEntry entry,
                                   final Level level, final Log log) {
        if (FORCE_INTERNAL_DEBUG) {
            debug("CommonsLoggingSessionLog.internalLog : "
                    + computeMessage(entry, level));
        }
        ...
        switch (entryLevel) {
            ...
            default:
                error("CommonsLoggingSessionLog.internalLog : unknown level : "
                        + entryLevel);
     ...
     * @return value
     */
    private final String computeMessage(final SessionLogEntry entry,
                                        final Level level) {
        // Format message so that we do not depend on the bundle
        final EclipseLinkLogRecord lr = new EclipseLinkLogRecord(level,
                formatMessage(entry));
    ...
    /**
     * INTERNAL: Return the corresponding java.util.logging.Level for a given
     * eclipselink level.
     *
     * @param level according to eclipselink level
     ...
     *
     * @see AbstractSessionLog#getLevelString() Return the log level as a string
     *      value.
     */
    ...
    private static final org.apache.log4j.Level getLevelFor(final int level) {
        ...
        switch (level) {
            ...
            default:
                realLevel = org.apache.log4j.Level.OFF;
                error("CommonsLoggingSessionLog.getLevelFor : unknown level : "
                        + getLevelString(level) + " = OFF");

                break;
        }
        if (FORCE_INTERNAL_DEBUG) {
            debug("CommonsLoggingSessionLog.getLevelFor : level : "
                    + getLevelString(level) + " = " + realLevel);
        }
        return realLevel;
    ...
    /**
     * INTERNAL: LogWrapper class wraps the real apache commons logging Log
     * instance
     */
    private static final class LogWrapper {
         ...
         */
        protected LogWrapper(final CommonsLoggingSessionLog sessionLog,
                             final String category, final Log log) {
            ...
            if ((parentName != null) && !"null".equals(parentName)) {
                if (FORCE_INTERNAL_DEBUG) {
                    debug("CommonsLoggingSessionLog.LogWrapper.new : parent : "
                            + parentName);
                }
        ...
        protected final void resetCachedLevel() {
            ...
            if (FORCE_INTERNAL_DEBUG) {
                debug("CommonsLoggingSessionLog.LogWrapper.setLevel : reset cachedLevel for : "
                        + getCategory());
            }
        }
        ...
        protected final int getLevel() {
            ...
            if (res == UNDEFINED_LEVEL) {
                ...
                if (FORCE_INTERNAL_DEBUG) {
                    System.out
                            .println("CommonsLoggingSessionLog.LogWrapper.getLevel : this : "
                                    + getCategory()
                                    + " : UNCACHED level : "
                                    + getLevelString(res));
                }
        ...
        protected final void addChild(final LogWrapper lw) {
            ...
            if (FORCE_INTERNAL_DEBUG) {
                System.out
                        .println("CommonsLoggingSessionLog.LogWrapper.addChild : this : "
                                + getCategory()
                                + " : child : "
                                + lw.getCategory());
            }
        ...
        private final int computeLevel(final int localLevel) {
            ...
            if (FORCE_INTERNAL_DEBUG) {
                System.out
                        .println("CommonsLoggingSessionLog.LogWrapper.computeLevel : IN : "
                                + getCategory()
                                + " : level : "
                                + CommonsLoggingSessionLog
                                .getLevelString(localLevel));
            }

            while ((lw != null)
                    && (((lw == this) && (localLevel == UNDEFINED_LEVEL)) || ((lw != this) && (lw
                    .getLevel() == UNDEFINED_LEVEL)))) {
                pw = lw.parent;
            ...
            if (lw != null) {
                ...
                if (FORCE_INTERNAL_DEBUG) {
                    System.out
                            .println("CommonsLoggingSessionLog.LogWrapper.computeLevel : category : "
                                    + lw.getCategory()
                                    + " - computedLevel : "
                                    + getLevelString(computedLevel)
                                    + " : "
                                    + lw.getLog());
                }
                final Logger logger = getLog4JLogger(lw.getLog());
                ...
                if (logger == null) {
                    error("CommonsLoggingSessionLog.computeLevel : Logger not found : "
                            + lw.getCategory() + " : " + lw.getLog());
                } else {
            ...
            if (FORCE_INTERNAL_DEBUG) {
                System.out
                        .println("CommonsLoggingSessionLog.LogWrapper.computeLevel : OUT : "
                                + getCategory()
                                + " : level : "
                                + CommonsLoggingSessionLog
                                .getLevelString(computedLevel));
