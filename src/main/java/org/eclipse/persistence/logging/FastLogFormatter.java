 ...
 * 
 * Contributors: Oracle - initial API and implementation from Oracle TopLink
 ******************************************************************************/
...
/**
 * <p>
 * Print a brief summary of a TopLink LogRecord in a human readable format. The
...
public final class FastLogFormatter extends SimpleFormatter {
	 ...
	 */
	private final static String LINE_SEPARATOR = PrivilegedAccessHelper
			.getLineSeparator();
		...
		if (!(pRecord instanceof EclipseLinkLogRecord)) {
			...
			if (record.getConnection() != null) {
				...
				sb.append("Connection").append(
						PAR_OPEN_CHAR);
				sb.append(
						String.valueOf(System.identityHashCode(record
								.getConnection()))).append(PAR_CLOSE_CHAR);
			}
			...
			if (record.shouldPrintThread()) {
				...
				sb.append("Thread").append(
						PAR_OPEN_CHAR);
				sb.append(String.valueOf(record.getThreadID())).append(
						PAR_CLOSE_CHAR);
			}
